...
export default function AdminSettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/settings')
      .then(async (r) => {
        ...
        if (r.ok) setWhatsapp(d?.data?.whatsappNumber || '');
      })
      .catch(()=>{});
  }, [checked]);
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch('/api/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ whatsappNumber: whatsapp }) });
